import React, { useState, useEffect } from "react";import { useLang } from "../context/LangContext";
...
const social = [
    ...
    {
        name: "Discord",
        url: "https://discord.gg/Kianmehr",
        color: "from-indigo-600/20 to-indigo-800/20 border-indigo-700/30",
    },
];
...
export default function Footer({ isDark }) {
    ...
    return (
        <footer className={`relative overflow-hidden border-t ${bdr}  ${bg}`}>
            ...
            <div
                className="absolute inset-0 bg-cover bg-center grayscale opacity-10 h-74"
                style={{
                    backgroundImage: `url(${BG_IMAGE})`,
                    transform: `rotateY(${ROTATE}deg) scale(1.05)`, // 3D rotate & scale
            ...
            <div className="relative z-10 max-w-6xl mx-auto px-6 py-16">
                <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3
                                className={`text-xl font-bold tracking-tight ${pey}`}>
                                Kianmehr
                            </h3>
                            <p className={`text-xs ${txt} ${pey}`}>
                                {T[lang].strategy}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        {social.map((s) => (
                            <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className={`group grid place-items-center w-10 h-10 rounded-full border ${s.color} hover:-translate-y-1 transition-all duration-300`}>
                                <span className="w-5 h-5 text-gray-200 group-hover:text-white transition" />
                            </a>
                        ))}
                        <button className="group grid place-items-center w-10 h-10 rounded-full border border-gray-700/50 bg-gray-800/40 hover:bg-gray-700/40 transition">
                            <svg
                                className={`w-5 h-5 ${
                                    isDark ? "text-yellow-500" : "text-gray-300"
                ...
                <div className="mt-10 text-center">
                    <p
                        className={`inline-flex items-center gap-2 text-sm ${txt} ${pey}`}>
                        {T[lang].crafted}{" "}
