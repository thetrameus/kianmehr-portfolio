...
export default function Navbar({ isDark, setIsDark }) {
    ...
    const bg = isDark
        ? "bg-gray-900/90 backdrop-blur-xl border-transparent border-gray-800"
        : "bg-white/90 backdrop-blur-xl border-transparent border-gray-200";
    const txt = isDark ? "text-gray-300" : "text-gray-700";

    return (
        <nav
            className={`sticky top-0 z-50 ${scrolled ? bg : "bg-transparent"} ${
                lang === "fa" ? "font-peyda" : ""
            }`}>
            <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
                ...
                {/* دسکتاپ منو */}
                <ul
                    className={`hidden md:flex items-center gap-6 text-sm ${txt}`}>
                    {navKeys.map((k) => (
                        <li key={k}>
                            <a
                                href={`#${k}`}
              ...
              ${
                  isDark
                    ...
                    <button
                        ...
                        className={`group grid place-items-center w-9 h-9 rounded-full border ${
                            isDark
                                ? "border-gray-700/50 bg-gray-800/40 hover:bg-gray-700/40"
