            ...
            {active === s.id && (
              <div className="mt-2 px-4 py-3 bg-black/50 backdrop-blur-sm border border-white/5 rounded-b-lg animate-fade">
                ...
                <div className="flex flex-wrap gap-1 mt-2">
                  {(isRTL ? s.skills.fa : s.skills.en).map((sk) => (
                    <span
                      key={sk}
          ...
          {active && (
            <>
              <img
                ...
                alt={
                  isRTL
                    ? sections.find((s) => s.id === active)?.title.fa
                    : sections.find((s) => s.id === active)?.title.en
                }
                className="w-full h-48 object-cover rounded"
              />
              ...
              <p className="text-xs text-gray-300 mt-2">
                {isRTL
                  ? sections.find((s) => s.id === active)?.text.fa
                  : sections.find((s) => s.id === active)?.text.en}
              </p>
              <div className="flex flex-wrap gap-1 mt-2">
                {(isRTL
                  ? sections.find((s) => s.id === active)?.skills.fa
                  : sections.find((s) => s.id === active)?.skills.en
                )?.map((sk) => (
                  <span
                    key={sk}
                    className="px-2 py-0.5 text-xs text-gray-400 border border-gray-600 rounded"
